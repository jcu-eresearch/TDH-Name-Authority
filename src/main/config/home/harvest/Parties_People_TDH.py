...
class IndexData:
    ...
    def __metadata(self):
        ...
        # Known IDs
            idFields = ["ID", "URI", "NLA_Party_Identifier", "ResearcherID", "openID", "Personal_URI", "Old_ID", "ORCID"]
    ...
    def __security(self, oid, index):
        ...
        if roles is not None:
            ...
            if  published is not None:
                #The researcher has published, make their details viewable to 'guest' access
                if  (published == 'true' and guestRole == 'false'):
                    self.utils.add(index, "security_filter", "guest")
        else:
            # Default to admin access if Null object returned
            #This change has been made to hide all JCU people, unless they have published.
            schema = self.utils.getAccessSchema("derby");
            schema.setRecordId(oid)
            schema.set("role", "admin")
            self.utils.setAccessSchema(schema, "derby")
            self.utils.add(index, "security_filter", "admin")
